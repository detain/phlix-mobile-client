...
**`src/components/`** syncplay subdir: `syncplay/{SyncPlayModal,SyncPlayOverlay}.tsx` (re-exported from `src/components/syncplay/index.ts`) are the SyncPlay group-watch UI wired into `PlayerScreen.tsx`, backed by `src/api/SyncPlayManager.ts` and the fan-out coordinator `src/syncplay/SyncPlayService.ts`. SyncPlay `onError` frames are rendered via `describeSyncPlayError` from `src/syncplay/syncplayErrors.ts`. This resolver is error-code-first: the catalog sentence for the wire code wins, then the server `message`, then a generic fallback.
...
**Wire contracts**: `@phlix/contracts` (`github:detain/phlix-contracts#v0.5.1`) and `@phlix/syncplay` (`github:detain/phlix-syncplay#v0.1.5`) are the pinned wire source — `src/types/playback.ts` re-exports `AudioTrack`/`SubtitleTrack` verbatim instead of declaring a local mirror, `TranscodeJob`/`TranscodeStatus` carry `dash_url: string | null` (server S59 restored what S11 removed; contracts v0.4.4+ requires the key ALWAYS present — null means the job published no `manifest.mpd`), and SyncPlay positions go on the wire in milliseconds (`toSyncPlayPositionMs` in `src/screens/PlayerScreen.tsx`) while state and the native player stay in seconds.
