...
- `@phlix/syncplay` is pinned (`github:detain/phlix-syncplay#v0.1.5`) and is a constants-only surface — take protocol constants from it rather than re-declaring them.
- Error frames are code-first: `SyncPlayService.handleErrorMsg` reads `error_code`, then legacy `code`, then `'UNKNOWN'` (parsed via `wireString`, never cast). The viewer-facing sentence comes from `describeSyncPlayError(code, message)` in `src/syncplay/syncplayErrors.ts` — catalog hit → server `message` → generic fallback. The catalog is a `Record<SyncPlayErrorCode, string>` (English-only, no i18n layer), so every contracts code must have an entry; `src/__tests__/syncplay/syncplayErrors.test.ts` pins it against `SYNCPLAY_ERROR_CODES`.
