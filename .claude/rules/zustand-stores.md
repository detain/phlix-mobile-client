...
- Persisted stores call `get().saveSettings()` inside each setter and load via `AsyncStorage.getItem(SETTINGS_KEY)` on `loadSettings()` — see `useSettingsStore.ts`. Storage key prefix: `phlex_`.
