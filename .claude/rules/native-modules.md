...
- **Android** (`android/app/src/main/java/com/phlexmobile/player/`): `PhlexPlayerView.kt` extends `ReactViewManager`, uses ExoPlayer (`Player.Listener` for state, `PlaybackException` for errors). Register props with `@ReactProp(name = "...")` and imperative methods with `@ReactMethod`. New packages must be added to `PhlexPlayerPackage.kt` and wired in `MainApplication.kt`.
