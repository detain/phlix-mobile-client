...
- Shapes `@phlix/contracts` already declares are re-exported, not re-declared: `src/types/playback.ts` re-exports `AudioTrack`/`SubtitleTrack` verbatim. `TranscodeJob`/`TranscodeStatus` stay a narrowed local mirror (consumer-shaped `subtitles`, legacy-tolerant `variants`) — and a mirror must mirror the TRUTH: `dash_url: string | null` is REQUIRED and always present (server S59 restored what S11 removed; contracts v0.4.4+ requires it), so never remove or optionalise the key.
