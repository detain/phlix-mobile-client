...
**`src/navigation/RootNavigator.tsx`**: `NavigationContainer` → root `Stack` (`Login` | `Main` + `Profiles`/`AdminDashboard`/`AdminUsers`/`AdminLibraries`/`LiveTv`/`LiveTvRecordings`/`Cast`/`Player`) → `Tab` (`Home` · `Library` · `Search` · `Downloads` · `Settings`). `Cast` is a headered root-stack screen taking params; `Player` presented as `fullScreenModal`. Auth gate reads `useAuthStore`.
