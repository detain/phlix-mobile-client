...
- **Wire types come from `@phlix/contracts`** (pinned `github:detain/phlix-contracts#v0.5.1`, alongside `@phlix/syncplay#v0.1.5`): `src/types/playback.ts` re-exports `AudioTrack`/`SubtitleTrack` verbatim rather than declaring a local mirror, and `TranscodeJob`/`TranscodeStatus` carry `dash_url: string | null` — the key is ALWAYS present (server S59 restored what S11 removed; contracts v0.4.4+ requires it; null = no `manifest.mpd` published). Do not remove or optionalise it.
...
- **SyncPlay errors are code-first**: render the viewer-facing sentence through `describeSyncPlayError(code, message)` in `src/syncplay/syncplayErrors.ts` (a `Record<SyncPlayErrorCode, string>` English catalog keyed by the exact wire code); the server's `message` is only a fallback. `SyncPlayService` reads `error_code` then `code`.
