...
// (snake_case) from `PhotoController` — see the E9 worklog "VERIFIED SERVER
// CONTRACTS → PHOTOS". Do NOT camelCase these.
//
// Image display: `thumbnail_url` / `full_url` (and slideshow `url`) are SIGNED
// URLs (token embedded). Use them directly in `<Image source={{ uri }}>` — no
// Bearer header, no hand-built URLs.

/**
 * Parsed EXIF for a photo (snake_case, server-verbatim). Numeric/string fields
 * may be absent or null on photos lacking metadata; the documented summary
 * fields (`orientation_name`, the formatted dates, `gps_display`) are present as
 * strings (possibly empty) on a PhotoDetail.
 */
 ...
 * `metadata` EXIF + SIGNED `thumbnail_url`/`full_url`). `metadata` is tolerant:
 * a partial EXIF object that may carry extra row fields.
 */
 ...
 */
export const formatExifSummary = (exif: Partial<PhotoExif>): string[] => {
  const lines: string[] = [];
...
/**
 * Numeric GPS coordinates for a photo, or null when either lat/lng is absent.
 */
export const gpsCoords = (
  exif: Partial<PhotoExif>
): { lat: number; lng: number } | null => {
  if (
    typeof exif.gps_lat === 'number' &&
    Number.isFinite(exif.gps_lat) &&
    typeof exif.gps_lng === 'number' &&
    Number.isFinite(exif.gps_lng)
  ) {
    return { lat: exif.gps_lat, lng: exif.gps_lng };
  }
  return null;
};
